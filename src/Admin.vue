<template>
  <div class="min-h-screen bg-black text-white font-ios selection:bg-white/20 overflow-x-hidden pb-40 text-left">
      ...
      <transition name="page" mode="out-in">
        ...
        <div v-else-if="activeTab === 'borrowers'" key="borrowers" class="py-10">
          ...
          <div v-for="person in borrowers" :key="person.id" 
               ...
               class="p-8 rounded-[2.5rem] mb-4 flex justify-between items-center border shadow-2xl transition-all">
            <div class="flex-1">
              <h3 class="text-lg font-black uppercase tracking-tighter leading-none italic mb-1">{{ person.bookTitle }}</h3>
            ...
            <button @click="markAsReturned(person)" 
                    class="px-8 py-5 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest bg-white text-black active:scale-95 transition-all">
              Return
            </button>
          </div>
        </div>
    ...
    <div class="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 w-full max-w-[440px] px-6">
      <nav class="bg-zinc-900/70 backdrop-blur-3xl border border-white/10 rounded-[2rem] p-2 flex items-center justify-between shadow-2xl">
        <button v-for="tab in ['dashboard', 'inventory', 'requests', 'borrowers', 'logs']" :key="tab" @click="activeTab = tab" 
                ...
                class="w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-500 relative">
          ...
          <div v-if="tab === 'requests' && pendingRequests.length > 0" class="absolute -top-1.5 -right-1.5 w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center text-[10px] font-black text-white border-2 border-black animate-pulse">{{ pendingRequests.length }}</div>
        </button>
      </nav>
    </div>
    ...
    <transition name="fade">
      <div v-if="showAddModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-2xl px-6">
        <div class="bg-zinc-950 border border-white/10 p-10 rounded-[3rem] max-w-sm w-full">
          ...
          <input v-model="newBook.title" type="text" placeholder="Entry Title" class="w-full bg-zinc-900 border border-white/5 rounded-2xl py-5 px-6 text-white outline-none font-bold placeholder:text-zinc-700 focus:border-blue-600 transition-all mb-10" @keyup.enter="addBook" />
          <div class="flex gap-3">
            <button @click="addBook" class="flex-[2] py-5 bg-white text-black rounded-2xl font-black uppercase text-[11px] active:scale-95 transition-all">Submit</button>
    ...
    <transition name="fade">
      <div v-if="showDeleteModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-2xl px-6">
        <div class="bg-zinc-950 border border-white/10 p-10 rounded-[3rem] max-w-xs w-full text-center">
          <h3 class="text-2xl font-black mb-10 uppercase italic tracking-tighter">De-register?</h3>
          <button @click="deleteSelectedBooks" class="w-full py-5 bg-red-600 text-white rounded-2xl font-black uppercase text-[11px] mb-3 active:scale-95 transition-all">Execute Purge</button>
          <button @click="showDeleteModal = false" class="w-full py-5 text-zinc-600 font-bold uppercase text-[10px]">Cancel</button>
        </div>
      </div>
    </transition>
...
/**
 * PINAKA-IMPORTANTENG PARTE:
 * Kapag pinindot ang Return, idadagdag muna sa history 
 * at saka idedelete sa borrowers collection.
 */
const markAsReturned = async (person) => {
  try {
    // 1. I-save sa history logs
    ...
    // 2. I-delete mula sa borrowers collection
    await deleteDoc(doc(db, "borrowers", person.id));
    
    console.log("Asset returned and removed from borrowers.");
  } catch (error) {
...
.standard-card { background: rgba(255,255,255,0.03); border-color: rgba(255,255,255,0.08); }
.overdue-card { background: rgba(220,38,38,0.1); border-color: rgba(220,38,38,0.3); }
