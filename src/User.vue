  ...
  <div class="min-h-screen bg-black text-white font-ios selection:bg-white/20 overflow-x-hidden pb-40">
    ...
    <header class="p-6 flex justify-between items-center relative z-20">
      ...
      <div class="flex items-center gap-2 bg-zinc-900 px-3 py-1.5 rounded-full border border-white/5">
        ...
        <span class="text-[9px] font-bold uppercase tracking-widest text-zinc-400">Sync Active</span>
      </div>
    </header>
    ...
    <main class="max-w-5xl mx-auto px-4 relative z-10">
      <transition name="page" mode="out-in">
        ...
        <div v-if="activeTab === 'home'" key="home" class="space-y-6 py-4">
          <section>
            <p class="text-zinc-600 text-[9px] font-bold uppercase tracking-[0.4em] mb-1">Navigation</p>
            <h2 class="text-4xl font-bold tracking-tighter uppercase italic">Dashboard</h2>
          </section>

          <div class="grid grid-cols-2 gap-3">
            <div class="bg-zinc-950 border border-white/5 p-6 rounded-3xl">
              <p class="text-zinc-500 text-[9px] font-bold uppercase tracking-widest mb-1">Books</p>
            ...
            <div class="bg-zinc-950 border border-white/5 p-6 rounded-3xl" @click="activeTab = 'notifications'">
              <p class="text-zinc-500 text-[9px] font-bold uppercase tracking-widest mb-1">Inbox</p>
          ...
          <section @click="activeTab = 'loans'" class="bg-zinc-900 border border-white/10 p-8 rounded-[2rem] cursor-pointer hover:bg-zinc-800 transition-colors">
             ...
             <p class="text-zinc-500 text-xs mb-4">You have {{ approvedRequests.length }} approved assets.</p>
        ...
        <div v-else-if="activeTab === 'explore'" key="explore" class="py-4">
          <div class="mb-4 sticky top-4 z-40">
            <div class="bg-zinc-900/95 backdrop-blur-xl border border-white/10 p-1 rounded-xl flex items-center">
              <input v-model="searchQuery" type="text" placeholder="Filter assets..." class="bg-transparent border-none w-full py-3 px-4 text-sm outline-none font-bold placeholder:text-zinc-700" />
            </div>
          </div>
            ...
            <div v-for="book in filteredBooks" :key="book.id" class="bg-zinc-950 border border-white/5 p-4 rounded-xl flex items-center justify-between">
              <div class="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p class="text-[8px] text-zinc-600 uppercase mt-1 tracking-widest">Global Asset</p>
                </div>
              </div>
        ...
        <div v-else-if="activeTab === 'loans'" key="loans" class="py-10 space-y-4">
          ...
          <div v-if="approvedRequests.length === 0" class="p-16 border border-dashed border-white/10 rounded-[2rem] text-center">
            <p class="text-zinc-800 font-bold uppercase text-[9px] tracking-widest">No Approved Records Found</p>
            <p class="text-zinc-900 text-[8px] mt-2">Check Inbox for status updates</p>
          </div>

          <div v-for="loan in approvedRequests" :key="loan.id" class="bg-white text-black p-6 rounded-2xl mb-3 flex justify-between items-center shadow-xl">
            <div class="max-w-[60%]">
            ...
            <div class="text-right">
              <p class="text-[8px] font-black uppercase tracking-widest mb-1 opacity-50">Due Date</p>
              <p class="text-lg font-mono font-bold text-blue-600 tabular-nums">
          ...
          <div v-for="req in sortedRequests" :key="req.id" class="p-4 bg-zinc-950 border border-white/5 rounded-xl flex items-center justify-between">
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <p class="text-[8px] uppercase font-black tracking-widest mt-0.5" :class="getTextStatusColor(req.status)">
                  Status: {{ req.status || 'Pending' }}
                </p>
              </div>
            </div>
            <span class="text-[8px] font-mono text-zinc-800 font-bold">{{ formatTimestamp(req.createdAt) }}</span>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'profile'" key="profile" class="py-10 space-y-10">
          <div class="text-center">
            ...
            <p class="text-zinc-600 text-[9px] font-bold uppercase tracking-[0.4em] mt-2">Verified Identity</p>
          </div>
          <div class="max-w-xs mx-auto space-y-3">
             <div class="bg-zinc-950 border border-white/5 p-4 rounded-2xl flex justify-between items-center">
                <span class="text-[9px] text-zinc-600 uppercase font-black">Status</span>
                <span class="text-[9px] text-green-500 uppercase font-black">Online</span>
             </div>
             <button @click="showLogoutModal = true" class="w-full py-4 bg-zinc-900 text-red-500 rounded-2xl font-black uppercase text-[10px] tracking-widest border border-red-500/10 active:bg-red-500 active:text-white transition-all">Terminate Session</button>
          </div>
        </div>
    ...
    <div class="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 w-full max-w-[340px] px-6">
      <nav class="bg-zinc-900/95 backdrop-blur-2xl border border-white/10 rounded-full p-1.5 flex items-center justify-between shadow-2xl">
    ...
    <transition name="fade">
      <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-md px-6">
        <div class="bg-zinc-950 border border-white/10 p-8 rounded-[2rem] max-w-sm w-full text-center">
          <h2 class="text-xl font-bold tracking-tighter mb-6 uppercase italic">Return Asset</h2>
          ...
          <button @click="submitRequest" :disabled="!returnDate || dateError !== ''" :class="(!returnDate || dateError !== '') ? 'bg-zinc-800 text-zinc-500' : 'bg-white text-black'" class="w-full py-4 rounded-xl font-black uppercase text-[10px] tracking-widest">Confirm Request</button>
    ...
    <transition name="fade">
      <div v-if="showLogoutModal" class="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 backdrop-blur-md px-6">
        <div class="bg-zinc-950 border border-white/10 p-10 rounded-[2.5rem] max-w-xs w-full text-center">
          <h3 class="text-xl font-black mb-6 uppercase tracking-tighter italic leading-none">Confirm Termination?</h3>
          <div class="flex gap-3">
            <button @click="showLogoutModal = false" class="flex-1 py-4 rounded-xl border border-white/10 font-bold text-zinc-500 text-[10px] uppercase">Abort</button>
    ...
    <transition name="slide-up">
      <div v-if="showToast" class="fixed top-8 left-1/2 -translate-x-1/2 bg-white text-black px-8 py-3 rounded-full font-black text-[9px] uppercase z-[150] shadow-2xl">Asset Logged</div>
    </transition>
...
onMounted(() => {
  ...
  // CRITICAL: Robust Authentication & Notification Sync
  onAuthStateChanged(auth, (user) => {
    if (user) {
      console.log("Logged in as:", user.uid);
      // Sinisiguro natin na pakikinggan ang lahat ng status (Approved, Pending, Declined)
      onSnapshot(query(collection(db, "notifications"), where("userId", "==", user.uid)), (s) => {
        userRequests.value = s.docs.map(d => ({ id: d.id, ...d.data() }));
        console.log("Sync success! Items found:", userRequests.value.length);
      }, (error) => {
...
// UPDATED FILTER: Check for 'approved' status case-insensitively
...
const isStatus = (current, target) => current?.toString().toLowerCase() === target.toLowerCase();
