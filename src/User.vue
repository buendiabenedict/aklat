  ...
  <div class="min-h-screen bg-black text-white font-ios selection:bg-white/20 overflow-x-hidden pb-40">
    ...
    <main class="max-w-5xl mx-auto px-6 relative z-10">
      <transition name="page" mode="out-in">
        ...
        <div v-if="activeTab === 'home'" key="home" class="space-y-10 py-6">
          ...
          <section v-if="approvedRequests.length > 0" class="space-y-6">
            <p class="text-blue-500 text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-2">
               <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
               Live Asset Tracking
            </p>
            <transition-group name="list">
              <div v-for="loan in approvedRequests" :key="loan.id" class="bg-gradient-to-br from-zinc-900 to-black border border-white/10 p-8 rounded-[2.5rem] relative overflow-hidden group shadow-2xl">
                <div class="relative z-10 flex flex-col sm:flex-row justify-between sm:items-end gap-6">
                  <div>
                    <h3 class="text-2xl font-bold tracking-tight mb-1 text-white">{{ loan.bookTitle }}</h3>
                    <p class="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">Return Date: {{ loan.returnSchedule }}</p>
                  </div>
                  <div class="bg-white/5 p-4 rounded-2xl border border-white/5 backdrop-blur-md">
                    <p class="text-zinc-500 text-[9px] font-black uppercase tracking-[0.2em] mb-1">Time to Return</p>
                    <p class="text-2xl font-mono font-bold text-blue-500 tabular-nums tracking-tight">
                      {{ calculateCountdown(loan.returnSchedule) }}
                    </p>
                  </div>
                </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="bg-zinc-950 border border-white/5 p-8 rounded-[2.5rem]">
              <p class="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-1">Catalog Size</p>
              <p class="text-4xl font-bold tracking-tighter text-white">{{ books.length }}</p>
            </div>
            <div class="bg-zinc-950 border border-white/5 p-8 rounded-[2.5rem]">
              <p class="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-1">Total Logs</p>
            ...
            <transition-group name="list">
              <div v-for="book in filteredBooks" :key="book.id" class="group bg-zinc-950 border border-white/5 p-10 rounded-[2.5rem] hover:border-white/20 transition-all duration-500 flex flex-col justify-between h-80">
                <div>
                  ...
                  <h3 class="text-2xl font-bold tracking-tight mb-2 italic">{{ book.title }}</h3>
        ...
        <div v-else-if="activeTab === 'notifications'" key="notifications" class="py-10 space-y-6">
          <h2 class="text-4xl font-bold tracking-tighter mb-10 italic">INBOX</h2>
          <transition-group name="list">
            <div v-if="userRequests.length === 0" key="empty" class="p-20 border border-dashed border-white/10 rounded-[3rem] text-center text-zinc-800 font-bold uppercase tracking-widest text-xs">Inbox Empty</div>
            <div v-for="req in sortedRequests" :key="req.id" class="p-8 bg-zinc-950 border border-white/5 rounded-3xl flex items-center justify-between group hover:border-white/10 transition-all mb-4">
              <div class="flex items-center gap-6">
                <div :class="getStatusColor(req.status)" class="w-12 h-12 rounded-2xl flex items-center justify-center transition-colors">
                  <svg v-if="req.status?.toLowerCase() === 'approved'" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" /></svg>
                  <svg v-else-if="req.status?.toLowerCase() === 'declined'" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12" /></svg>
                  <svg v-else class="w-6 h-6 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                </div>
                <div>
                  <h4 class="font-bold tracking-tight text-lg">{{ req.bookTitle }}</h4>
                  <p class="text-[10px] uppercase font-black tracking-[0.2em] mt-1" :class="getTextStatusColor(req.status)">
                    Request {{ req.status || 'Processing' }}
                  </p>
                </div>
              </div>
        ...
        <div v-else-if="activeTab === 'profile'" key="profile" class="py-10 space-y-8">
          ...
          <div class="bg-zinc-950 border border-white/5 rounded-[3rem] p-10 space-y-8 shadow-2xl">
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="text-green-500 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border border-green-500/20 font-bold">Authorized</span>
            </div>
          </div>
        </div>
    ...
    <div class="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 w-full max-w-[400px] px-6">
      <nav class="bg-zinc-900/90 backdrop-blur-3xl border border-white/10 rounded-full p-2.5 flex items-center justify-between shadow-2xl">
        <button @click="activeTab = 'home'" :class="activeTab === 'home' ? 'bg-white text-black scale-110' : 'text-zinc-500'" class="flex items-center justify-center w-12 h-12 rounded-full transition-all duration-500">
        ...
        <button @click="activeTab = 'explore'" :class="activeTab === 'explore' ? 'bg-white text-black scale-110' : 'text-zinc-500'" class="flex items-center justify-center w-12 h-12 rounded-full transition-all duration-500">
        ...
        <button @click="activeTab = 'notifications'" :class="activeTab === 'notifications' ? 'bg-white text-black scale-110' : 'text-zinc-500'" class="flex items-center justify-center w-12 h-12 rounded-full transition-all duration-500 relative">
        ...
        <button @click="activeTab = 'profile'" :class="activeTab === 'profile' ? 'bg-white text-black scale-110' : 'text-zinc-500'" class="flex items-center justify-center w-12 h-12 rounded-full transition-all duration-500">
    ...
    <transition name="fade">
      <div v-if="showLogoutModal" class="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 backdrop-blur-md px-6 text-center">
        <div class="bg-zinc-950 border border-white/10 p-12 rounded-[3.5rem] max-w-sm w-full">
          <h3 class="text-3xl font-bold mb-3 tracking-tighter italic uppercase">Sign Out</h3>
...
// IMPROVED: Filter approved requests (Case-Insensitive)
...
onMounted(() => {
  ...
  // CRITICAL FIX: Ensure real-time monitoring of notifications
  if (auth.currentUser) {
    console.log("Monitoring notifications for UID:", auth.currentUser.uid);
    onSnapshot(query(
      collection(db, "notifications"), 
      where("userId", "==", auth.currentUser.uid)
    ), (s) => {
      userRequests.value = s.docs.map(d => ({ id: d.id, ...d.data() }));
      console.log("Updated Notifications:", userRequests.value);
    });
  }
...
const getStatusColor = (status) => {
  ...
  if (s === 'approved') return 'bg-green-500/10 text-green-500 border border-green-500/20';
  if (s === 'declined') return 'bg-red-500/10 text-red-500 border border-red-500/20';
