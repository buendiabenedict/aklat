<template>
  <div class="min-h-screen bg-black text-white font-ios selection:bg-white/20 overflow-x-hidden pb-40">
      ...
      <transition name="page" mode="out-in">
        ...
        <div v-if="activeTab === 'home'" key="home" class="space-y-6 py-4">
          <section>
            ...
            <h2 class="text-4xl font-bold tracking-tighter uppercase">Terminal</h2>
          </section>

          <div class="grid grid-cols-2 gap-3">
            <div class="bg-zinc-950 border border-white/5 p-6 rounded-3xl">
              <p class="text-zinc-500 text-[9px] font-bold uppercase tracking-widest mb-1">Database</p>
            ...
            <div class="bg-zinc-950 border border-white/5 p-6 rounded-3xl" @click="activeTab = 'notifications'">
              <p class="text-zinc-500 text-[9px] font-bold uppercase tracking-widest mb-1">Inbound</p>
              <p class="text-3xl font-bold tracking-tighter text-blue-500">{{ userRequests.length }}</p>
            </div>
          </div>

          <section @click="activeTab = 'loans'" class="bg-zinc-900 border border-white/10 p-8 rounded-[2rem] cursor-pointer">
             <h3 class="text-2xl font-black tracking-tighter mb-1 italic uppercase">Active Loans</h3>
             <p class="text-zinc-500 text-xs mb-4">Tracking {{ approvedRequests.length }} confirmed assets.</p>
             <div class="w-8 h-8 bg-white rounded-full flex items-center justify-center text-black font-bold text-xs">{{ approvedRequests.length }}</div>
          </section>
        </div>
        ...
        <div v-else-if="activeTab === 'explore'" key="explore" class="py-4">
          ...
          <div class="space-y-2">
            <div v-for="book in filteredBooks" :key="book.id" class="bg-zinc-950 border border-white/5 p-4 rounded-xl flex items-center justify-between hover:border-white/20 transition-colors">
              <div class="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p class="text-[8px] text-zinc-600 uppercase mt-1 tracking-widest">Available Asset</p>
                </div>
              </div>
              <button @click="openBorrowModal(book)" class="px-4 py-2 bg-zinc-900 text-[9px] text-white hover:bg-white hover:text-black rounded-lg font-black uppercase tracking-widest transition-all">Borrow</button>
            </div>
          </div>
        </div>

        <div v-else-if="activeTab === 'loans'" key="loans" class="py-10 space-y-4">
          <h2 class="text-3xl font-bold tracking-tighter mb-6 italic uppercase">Confirmed Assets</h2>
          <div v-if="approvedRequests.length === 0" class="p-16 border border-dashed border-white/10 rounded-[2rem] text-center text-zinc-800 font-bold uppercase text-[9px]">No Active Loans Found</div>
          <div v-for="loan in approvedRequests" :key="loan.id" class="bg-white text-black p-6 rounded-2xl mb-3 flex justify-between items-center">
            <div>
              <p class="text-[8px] font-black uppercase tracking-widest mb-1 opacity-50">Authorized Asset</p>
              <h3 class="text-lg font-bold tracking-tighter uppercase leading-none">{{ loan.bookTitle }}</h3>
            </div>
            <div class="text-right">
              <p class="text-[8px] font-black uppercase tracking-widest mb-1 opacity-50">Due in</p>
              <p class="text-lg font-mono font-bold text-blue-600">{{ calculateCountdown(loan.returnSchedule) }}</p>
            </div>
          </div>
        </div>

        <div v-else-if="activeTab === 'notifications'" key="notifications" class="py-10 space-y-3">
          <h2 class="text-3xl font-bold tracking-tighter mb-6 italic uppercase">Logs</h2>
          <div v-for="req in sortedRequests" :key="req.id" class="p-4 bg-zinc-950 border border-white/5 rounded-xl flex items-center justify-between">
            <div class="flex items-center gap-4">
              <div :class="getStatusIconClass(req.status)" class="w-8 h-8 rounded-lg flex items-center justify-center">
                <svg v-if="isStatus(req.status, 'approved')" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-width="3" d="M5 13l4 4L19 7" /></svg>
                <svg v-else-if="isStatus(req.status, 'declined')" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-width="3" d="M6 18L18 6M6 6l12 12" /></svg>
                <svg v-else class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              </div>
              <div>
                <h4 class="font-bold tracking-tight text-xs uppercase">{{ req.bookTitle }}</h4>
                <p class="text-[8px] uppercase font-black tracking-widest" :class="getTextStatusColor(req.status)">{{ req.status || 'Pending' }}</p>
              </div>
            </div>
          </div>
        </div>

        <div v-else-if="activeTab === 'profile'" key="profile" class="py-10 space-y-6">
          <div class="text-center">
            ...
            <h2 class="text-2xl font-bold tracking-tighter">{{ auth.currentUser?.email }}</h2>
            <p class="text-zinc-600 text-[9px] font-bold uppercase tracking-[0.4em] mt-2">Authorized User</p>
          </div>
          <div class="max-w-xs mx-auto pt-10">
            <button @click="showLogoutModal = true" class="w-full py-4 bg-zinc-900 border border-white/5 text-red-500 rounded-2xl font-black uppercase text-[10px] tracking-widest">Sign Out</button>
          </div>
        </div>
    ...
    <div class="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 w-full max-w-[360px] px-6">
      <nav class="bg-zinc-900 border border-white/10 rounded-full p-1.5 flex items-center justify-around shadow-2xl">
        <button @click="activeTab = 'home'" :class="activeTab === 'home' ? 'bg-white text-black' : 'text-zinc-600'" class="w-10 h-10 rounded-full flex items-center justify-center transition-colors">
        ...
        <button @click="activeTab = 'explore'" :class="activeTab === 'explore' ? 'bg-white text-black' : 'text-zinc-600'" class="w-10 h-10 rounded-full flex items-center justify-center transition-colors">
        ...
        <button @click="activeTab = 'loans'" :class="activeTab === 'loans' ? 'bg-white text-black' : 'text-zinc-600'" class="w-10 h-10 rounded-full flex items-center justify-center transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
        </button>
        <button @click="activeTab = 'notifications'" :class="activeTab === 'notifications' ? 'bg-white text-black' : 'text-zinc-600'" class="w-10 h-10 rounded-full flex items-center justify-center transition-colors relative">
        ...
        <button @click="activeTab = 'profile'" :class="activeTab === 'profile' ? 'bg-white text-black' : 'text-zinc-600'" class="w-10 h-10 rounded-full flex items-center justify-center transition-colors">
    ...
    <transition name="fade">
      <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-md px-6">
        <div class="bg-zinc-950 border border-white/10 p-8 rounded-[2rem] max-w-sm w-full text-center">
          <h2 class="text-xl font-bold tracking-tighter mb-6 uppercase italic">Schedule Return</h2>
          ...
          <button @click="submitRequest" :disabled="!returnDate || dateError !== ''" :class="(!returnDate || dateError !== '') ? 'bg-zinc-800 text-zinc-500 cursor-not-allowed' : 'bg-white text-black'" class="w-full py-4 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all">Submit Request</button>
    ...
    <transition name="fade">
      <div v-if="showLogoutModal" class="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 backdrop-blur-md px-6">
        <div class="bg-zinc-950 border border-white/10 p-10 rounded-[2.5rem] max-w-xs w-full text-center">
          <h3 class="text-xl font-black mb-6 uppercase tracking-tighter italic">Terminate Session?</h3>
    ...
    <transition name="slide-up">
      <div v-if="showToast" class="fixed top-8 left-1/2 -translate-x-1/2 bg-white text-black px-8 py-3 rounded-full font-black text-[9px] uppercase z-[150]">Success</div>
    </transition>
...
onMounted(() => {
  ...
  // CRITICAL: Real-time Notifications Listener
  if (auth.currentUser) {
...
const approvedRequests = computed(() => userRequests.value.filter(req => req.status?.toLowerCase() === 'approved'));
...
const getStatusIconClass = (status) => isStatus(status, 'approved') ? 'bg-green-500/10 text-green-500' : isStatus(status, 'declined') ? 'bg-red-500/10 text-red-500' : 'bg-zinc-800 text-zinc-500';
const getTextStatusColor = (status) => isStatus(status, 'approved') ? 'text-green-500' : isStatus(status, 'declined') ? 'text-red-500' : 'text-zinc-600';
